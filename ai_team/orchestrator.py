...
{_read_doc(output_dir, 'sprint_plan.md')}""" if has_scrum else ""
        ...
        repo_note = f"\nRepo hiện có: {repo_url}" if repo_url and fs_agents else ""
...
{chr(10).join(task_files)}"""
    ...
    # Stage 4c: Fullstack agents (song song)
    fs_tasks = []
    if _enabled("Fullstack Agent 1"):
        fs_tasks.append(_coding_agent("Fullstack Agent 1", "fs1_task.md", out / "fullstack" / "fs1", output_dir))
    if _enabled("Fullstack Agent 2"):
        fs_tasks.append(_coding_agent("Fullstack Agent 2", "fs2_task.md", out / "fullstack" / "fs2", output_dir))
